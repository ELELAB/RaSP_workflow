...
#    with open(
#        f"output/ds_models/ds_model_{model_idx}/lc_results.pkl",
#        "wb",
#    ) as f:
#        pickle.dump(lc_results, f)
#    torch.save(
#        ds_model_net.state_dict(),
#        f"output/ds_models/ds_model_{model_idx}/model.pt",
#    )
"""

def ds_pred(
    cavity_model_net, ds_model_net, df_total, dataset_key, NUM_ENSEMBLE, DEVICE, path
):
    """
            ...
            for i in range(NUM_ENSEMBLE):
                ...
                if DEVICE == 'cpu':
                    ds_model_net.load_state_dict(
                        torch.load(
                            f"{path}/ds_models/ds_model_{model_idx}/model.pt", #added ,
...
def compute_pdb_combo_corrs(df, dataset_key):
    ...
    with open(
        f"output/{dataset_key}/combo_corr_results.pkl",
        "wb",
    ) as f:
