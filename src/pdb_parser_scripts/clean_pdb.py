...
def _step_1_reduce(
    ...
    command = [
        ...
        "-BUILD",
        "-DB",
        os.path.join(f"{path}/pdb_parser_scripts/reduce/reduce_wwPDB_het_dict.txt"),
        "-Quiet",
        pdb_input_filename,
    ]
    error_code = subprocess.Popen(command, stdout=temp1).wait()
    temp1.flush()
    first_model = PDB_PARSER.get_structure(pdbid, temp1.name)

    return first_model


def _step_3_pdbfixer(first_model, temp3):
#these lines have been updated 28 nov 2022 (KD)
    for model in first_model:
        for chain in model:
...
def clean_pdb(pdb_input_filename: str, out_dir: str, reduce_executable: str, path: str):
    """
    ...
    with tempfile.NamedTemporaryFile(mode="wt", delete=True) as temp1:
        first_model = _step_1_reduce(reduce_executable, pdb_input_filename, pdbid, temp1, path)
        ...
        with tempfile.NamedTemporaryFile(mode="wt", delete=True) as temp2:
            ...
            temp2.flush()
            first_model = PDB_PARSER.get_structure(temp2.name, temp2.name)
...
if __name__ == "__main__":
    ...
    # Clean 
    clean_pdb(pdb_input_filename, out_dir, reduce_executable, path)
    t1 = time.time()
